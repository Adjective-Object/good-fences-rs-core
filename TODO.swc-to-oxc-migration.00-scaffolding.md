...
- [x] Add OXC git dependencies to root `Cargo.toml` `[workspace.dependencies]`
- [x] Verify `cargo metadata` resolves the new graph (run `cargo check --workspace`)
...
- [x] Create the crate with `Cargo.toml` depending on `oxc_allocator`, `oxc_ast`, `oxc_parser`, `oxc_span`
- [x] Implement `parse_file`, `parse_ts`, `parse_tsx` in `lib.rs`
- [x] Implement `leading_comments_at(comments, source, statement_start)` using `partition_point` + a backward walk that stops once the gap between two adjacent comments contains a non-whitespace byte
- [x] Add unit test: parse a `.ts` and a `.tsx` fixture; assert `errors.is_empty()`
- [x] Add unit test: source `"// @ALLOW-UNUSED-EXPORT\nexport const x = 1;"` — parse, find the `Statement::ExportNamedDeclaration` in `program.body`, take its `.span.start`, assert `leading_comments_at(&program.comments, source, span.start)` returns one comment with `text` containing `"@ALLOW-UNUSED-EXPORT"`
- [x] Add unit test: source with an interior comment between two statements — assert that comment is NOT returned as leading for either neighbor (whichever rule we pick: it leads the second one if there's no blank-line gap, else neither). Document the rule we picked in a doc comment on `leading_comments_at`.

## Verify

- [x] `cargo check --workspace` succeeds
- [x] `cargo test -p oxc_utils_parse` passes
