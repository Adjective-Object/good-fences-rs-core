...
pub struct Segment {
    // external names used by this segment
    //
    // e.g. names from the scope the segment is defined in
    // that are not shadowed within the segment itself
    references_names: Vec<String>,

    // names defined by this segment in the scope it is defined in
    defines_names: Vec<String>,

    // If this segment is "effectful" according to static analysis
    // of the source of the segment. Note that actual effect analysis
    // requires also looking at the transitive syncronous dependencies
    // of this segment, so this should not be used directly to determine
    // if a module is effectful.
    //
    // E.g. creating the _definitiions_ within this statement is effectful,
    // a pire function definition is not effectful, even if its body is.
    has_own_effects: bool,

    // segnent-dependent import/export information from this segment
    import_info: SegmentImportInfo,
}
...
enum SegmentImport {
    ImportStatement(StaticImportType),
    ImportExpr(StaticImportType),
    RequireExpr(StaticImportType),
}
...
enum SegmentType {
    ...
    NormalSegment(SegmentImportInfo),
}
