...
use crate::parse::{ExportedItem, RawImportExportInfo};
...
pub enum SourceFileParseError {
    ...
    LoadFileError(PathBuf),
    #[error("TypeScript syntax error: {0}")]
    TypeScriptSyntaxError(PathBuf),
...
pub fn get_file_import_export_info(
    ...
    let fm = match cm.load_file(file_path) {
        ...
        Err(e) => {
            return Err(
                anyhow!(SourceFileParseError::LoadFileError(file_path.to_path_buf())).context(e),
            )
        }
    };
    ...
    if !errors.is_empty() {
        let mut error =
            anyhow::Error::new(SourceFileParseError::ParserError(file_path.to_path_buf()));
        for e in errors {
