...
pub fn walk_src_files(
    ...
    std::thread::scope(|scope| -> Result<(), anyhow::Error> {
        let collector_thread = scope.spawn(move || {
            for file in rx {
                println!("collector_thread collecitng {:#?}", file);
                match file {
                    Ok(file) => {
        ...
        println!("waiting for collector thread..");
...
fn visit_entry(entry: DirEntry) -> Result<Option<WalkedFile>, anyhow::Error> {
    println!("visiting entry {}", entry.path().display());
    ...
    if equals_os_str(file_name, "package.json") {
        ...
        get_file_import_export_info(entry.path()).map(|import_export_info| {
